import React from 'react'
import displayimg1 from "../component/img/displayimg1.png"
import displayimg2 from "../component/img/displayimg2.png"
import displayimg3 from "../component/img/displayimg3.png"
import displayimg4 from "../component/img/displayimg4.png"
import displayimg5 from "../component/img/displayimg5.png"
import displayimg6 from "../component/img/displayimg6.png"
import displayimg7 from "../component/img/displayimg7.png"
import displayimg8 from "../component/img/displayimg8.png"
import star2 from "../component/img/Star 2.png"


const Display = () => {
  return (
    <body className='body'>
        <div className='style-h1'> <h1> Inspiration for your next adventure</h1></div>
        <div className='img-div'> 
            <div className='box-mage'>
                <div className='img'><img className='img' src={displayimg1} alt="" /></div>
                <div className='write-up'>
                    <h3> Desert King <br /> 2345km away</h3>
                    <h3> 1MBT PER night <br /> available 2weeks stay </h3>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
            <div className='box-mage'>
                <div className='img'> <img src={displayimg2} alt="" /></div>
                <div className='write-up'>
                    <div>
                    <h3> Desert King </h3>
                    <h4> 2345km away</h4>
                    </div>
                    <div>
                    <h3> 1MBT PER night   </h3>
                    <h4>available 2weeks stay </h4>
                    </div>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
            <div className='box-mage'>
                <div className='img'> <img src={displayimg3} alt="" /></div>
                <div className='write-up'>
                    <h3> Desert King <br /> 2345km away</h3>
                    <h3> 1MBT PER night <br /> available 2weeks stay </h3>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
            <div className='box-mage'>
                <div className='img'> <img src={displayimg4} alt="" /></div>
                <div className='write-up'>
                    <h3> Desert King <br /> 2345km away</h3>
                    <h3> 1MBT PER night <br /> available 2weeks stay </h3>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
            <div className='box-mage'>
                <div className='img'> <img src={displayimg5} alt="" /></div>
                <div className='write-up'>
                    <h3> Desert King <br /> 2345km away</h3>
                    <h3> 1MBT PER night <br /> available 2weeks stay </h3>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
            <div className='box-mage'>
                <div className='img'> <img src={displayimg6} alt="" /></div>
                <div className='write-up'>
                    <h3> Desert King <br /> 2345km away</h3>
                    <h3> 1MBT PER night <br /> available 2weeks stay </h3>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
            <div className='box-mage'>
                <div className='img'> <img src={displayimg7} alt="" /></div>
                <div className='write-up'>
                    <h3> Desert King <br /> 2345km away</h3>
                    <h3> 1MBT PER night <br /> available 2weeks stay </h3>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
            <div className='box-mage'>
                <div className='img'> <img src={displayimg8} alt="" /></div>
                <div className='write-up'>
                    <h3> Desert King <br /> 2345km away</h3>
                    <h3> 1MBT PER night <br /> available 2weeks stay </h3>
                </div>
                <div className='star-img'>
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                        <img src={star2} alt="" />
                    </div>
            </div>
        </div>
    </body>
    
  )
}

export default Display